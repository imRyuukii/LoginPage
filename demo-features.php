<?php
// demo-features.php — UX demo page with global navbar
// Basic session and headers (optional for static page)
...
session_start();
require_once __DIR__ . '/src/app/security/headers.php';
...
<html lang="en">
  <head>
    ...
    <script src="/LoginPage/src/public/js/form-utils.js?v=<?php echo filemtime(__DIR__ . '/src/public/js/form-utils.js'); ?>" defer></script>
  </head>
  <body>
    <?php $NAV_BASE = '/LoginPage/'; include __DIR__ . '/src/public/partials/navbar.php'; ?>
    ...
    <div class="container page">
      ...
      <div class="card">
        ...
        <div class="cta-group" style="margin-top: 20px">
          <button class="button" onclick="Toast.success('Operation completed successfully! ✓', 5000)" style="border-color: rgba(34, 197, 94, 0.45)">Show Success</button>
          <button class="button" onclick="Toast.error('An error occurred. Please try again.', 5000)" style="border-color: rgba(239, 68, 68, 0.45)">Show Error</button>
          <button class="button" onclick="Toast.warning('Please verify your email address before continuing.', 5000)" style="border-color: rgba(245, 158, 11, 0.45)">Show Warning</button>
          <button class="button" onclick="Toast.info('Your session will expire in 5 minutes.', 5000)" style="border-color: rgba(99, 102, 241, 0.45)">Show Info</button>
        </div>
        <div class="cta-group" style="margin-top: 12px">
          <button class="button" onclick="showMultipleToasts()">Show Multiple Toasts</button>
          <button class="button" onclick="Toast.clearAll()">Clear All Toasts</button>
        </div>
        <div style="margin-top: 20px; padding: 12px; background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.25); border-radius: 10px;">
          ...
          <ul style="margin: 8px 0 0 0; padding-left: 20px; color: var(--muted);">
            ...
            <li>Manual close with × button</li>
      ...
      <div class="card">
        ...
        <form id="demoLoadingForm" onsubmit="return handleLoadingDemo(event)" style="margin-top: 20px">
          ...
          <div class="link-row centered mt-4">
            <button class="button primary" type="submit" id="loadingBtn">Submit Form</button>
            <button class="button" type="button" onclick="simulateManualLoading()">Manual Loading Test</button>
          </div>
        </form>
        <div style="margin-top: 20px; padding: 12px; background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.25); border-radius: 10px;">
          <strong style="color: var(--primary)">Features:</strong>
      ...
      <div class="card">
        ...
        <form id="validationDemoForm" onsubmit="return handleValidationDemo(event)" style="margin-top: 20px">
          ...
          <div class="link-row centered mt-4">
            <button class="button primary" type="submit">Validate & Submit</button>
            <button class="button" type="button" onclick="resetValidationForm()">Reset Form</button>
          </div>
        </form>
        <div style="margin-top: 20px; padding: 12px; background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.25); border-radius: 10px;">
          <strong style="color: var(--primary)">Features:</strong>
      ...
      <div class="card">
        ...
        <div style="margin-top: 20px">
          ...
          <ul style="margin: 0; padding-left: 20px; color: var(--muted); font-size: 14px;">
            ...
            <li><code>meta name="theme-color"</code> - Brand color (#124e66)</li>
        ...
        <div class="link-row centered mt-4">
          <button class="button" onclick="viewPageSource()">View Page Source</button>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div style="margin-top: 20px; padding: 14px; background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.35); border-radius: 10px;">
          <div style="display: flex; align-items: center; gap: 10px">
            ...
            <div>
              <strong style="color: #d3d9d4">All Features Implemented Successfully!</strong><br />
              <span style="font-size: 14px; color: var(--muted)">Total JS: ~15.5 KB | Total CSS: ~7 KB | Zero external dependencies</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="link-row centered mt-3">
          <a class="button" href="./src/docs/ux-enhancements.md" target="_blank">📖 Full Documentation</a>
          <a class="button" href="./src/docs/site-review.md" target="_blank">🔍 Code Review</a>
          <a class="button" href="./README.md" target="_blank">📋 Project README</a>
        </div>
      </div>
    </div>
    ...
    <script>
      ...
      function simulateManualLoading() {
        ...
        if (window.FormUtils) {
          ...
          setTimeout(() => { FormUtils.removeLoading(form, btn); Toast.success("Loading state removed!", 3000); }, 3000);
        }
      }
      ...
      function handleValidationDemo(event) {
        ...
        let valid = true; let errors = [];
        if (username.length < 3) { errors.push("Username too short"); valid = false; }
        if (!FormUtils.isValidEmail(email)) { errors.push("Invalid email format"); valid = false; }
        const passwordCheck = FormUtils.validatePasswordStrength(password);
        if (!passwordCheck.valid) { errors.push("Password too weak"); valid = false; }
        if (password !== confirm) { errors.push("Passwords do not match"); valid = false; }
        if (valid) { Toast.success("✓ All fields are valid! Form would be submitted in production.", 5000); }
        else { Toast.error("✗ Please fix validation errors: " + errors.join(", "), 6000); }
        return false;
      }
      ...
      function resetValidationForm() {
        ...
        inputs.forEach((input) => { input.classList.remove("input-valid", "input-invalid"); });
